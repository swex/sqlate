...
public:
     ...
     */
    explicit SchemaUpdaterBase(int targetVersion, const QString &pluginName = QString(),
                               QWidget *parentWidget = 0, QObject *parent = 0);
...
{
    explicit missing_column_creator(const QSqlDatabase &db)
        : m_db(db)
    {
    }
    ...
    void operator()(const Table &table)
    {
        ...
        boost::mpl::for_each<typename Table::columns,
                             Sql::detail::wrap<boost::mpl::placeholders::_1>>(nameAccu);

        QStringList columnStatements;
        Sql::detail::column_creator stmtAccu(columnStatements);
        boost::mpl::for_each<typename Table::columns,
                             Sql::detail::wrap<boost::mpl::placeholders::_1>>(stmtAccu);

        Q_ASSERT(columnNames.size() == columnStatements.size());
        for (QStringList::const_iterator it = columnNames.constBegin(),
                                         it2 = columnStatements.constBegin();
             it != columnNames.constEnd(); ++it, ++it2) {
            bool found = false;
            ...
            QString alterStmt = QLatin1Literal("ALTER TABLE ") % Table::tableName()
                % QLatin1Literal(" ADD COLUMN ") % *it2;
            SqlQuery query(m_db);
 ...
 * Additionally to SchemaUpdaterBase this also adds missing tables and columns based on the given
 * schema.
 */
...
public:
     ...
     * @param parentWidget widget used as dialog parent
     */
    explicit SchemaUpdater(int targetVersion, const QString &pluginName = QString(),
                           QWidget *parentWidget = 0, QObject *parent = 0)
    ...
    bool needsUpdate() const
    {
        ...
        try {
            ...
            emit errorMessage(
                QObject::tr(
                    "<html>There was an error while checking the database version. This is a critical error, the application will terminate.<br/>\
                                 The error was: <b>%1</b>.</html>")
                    .arg(e.error().text()));
            QApplication::exit(1);
            return false;
        }
    ...
    void execUpdates(bool interactive)
    {
        ...
        if (interactive
            && QMessageBox::warning(
                   m_parentWidget, QObject::tr("Database Schema Update Required"),
                   QObject::tr("An update to the database schema is required to proceed. Such an "
                               "update can take considerable time and will require updates to all "
                               "client installations as well. "
                               "Also, ensure you have current backups before proceeding."),
        ...
        try {
            ...
            emit errorMessage(
                QObject::tr("Database schema update failed during creation of new tables: %1.")
                    .arg(e.error().text()));
            QApplication::exit(1);
            return;
        }

        // step 2 auto-create missing columns, if the previous step succeeded this must not fail due
        // to missing tables
        try {
            createMissingColumns();
        } catch (const SqlException &e) {
            emit errorMessage(
                QObject::tr("Database schema update failed during creation of new columns: %1.")
                    .arg(e.error().text()));
            QApplication::exit(1);
            return;
        }
        ...
        foreach (
            ...
            pendingUpdates(QDir(QString::fromLatin1(":/%1schemaUpdates/").arg(m_pluginName)))) {
            try {
                emit infoMessage(
                    QObject::tr("Upgrading schema to version %1...").arg(update.version));
                SqlTransaction t;
                ...
                emit successMessage(
                    QObject::tr("Schema successfully upgraded to version %1").arg(update.version));
            } catch (const SqlException &e) {
                emit errorMessage(
                    QObject::tr("Schema update failed to version %1. The error was '%2'.")
                        .arg(update.version)
                        .arg(e.error().text()));
                QApplication::exit(1);
                return;
        ...
        // step 3 create the rules/triggers for tables. Must be done at the end, after the tables
        // are fully updated
        try {
        ...
        // step 4: if necessary increase the version number beyond what the custom updates did, in
        // case the latest version only included automatic changes.
        if (m_currentVersion < m_targetVersion) {
            try {
                ...
                emit errorMessage(
                    QObject::tr("The database schema version number could not be updated: %1.")
                        .arg(e.error().text()));
                QApplication::exit(1);
                return;
            }
...
protected:
    void createMissingTables() { Sql::createMissingTables<Schema>(QSqlDatabase::database()); }

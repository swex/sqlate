...
private Q_SLOTS:
    ...
    void testQueryBuilder_data()
    {
        ...
        QTest::newRow("Default values")
            << qb << "INSERT INTO tblReport DEFAULT VALUES" << columns << values;
        ...
        columns << QL1S("first");
        values << QL1S("1");
        QTest::newRow("1 column, string")
            << qb << "INSERT INTO table1 (first) VALUES (:0)" << columns << values;
        ...
        QTest::newRow("2 columns, string and number")
            << qb << "INSERT INTO table1 (first,second) VALUES (:0,:1)" << columns << values;
        ...
        QTest::newRow("2 columns, string and number, number is NULL")
            << qb << "INSERT INTO table1 (first,second) VALUES (:0,:1)" << columns << values;
        ...
        QTest::newRow("1 column, uuid, template")
            << qb << "INSERT INTO tblPerson (id) VALUES (:0)" << columns << values;
        ...
        values << QVariant::fromValue(hoId) << QVariant::fromValue<SqlNowType>(SqlNow)
               << QVariant();
        QTest::newRow("server-side now")
            << qb << "INSERT INTO tblReport (id,ts,txt) VALUES (:0,now(),:2)" << columns << values;
    }
    ...
    void testQueryBuilder()
    {
        ...
        QVERIFY(
            std::equal(qb.m_values.begin(), qb.m_values.end(), values.begin(), deepVariantCompare));
    }
};

...
void TestBase::openDbTest()
{
    ...
    if (!db.open()) {
        ...
        QString s =
            QString(QLatin1String("CREATE DATABASE %1 ENCODING 'UTF8' LC_COLLATE='en_US.UTF-8' "
                                  "LC_CTYPE='en_US.UTF-8' TEMPLATE template0"))
                .arg(databaseName());
        QSqlQuery query(db2);
        if (!query.exec(s)) {
            qFatal("Failed to create the %s database: %s", databaseName().toLatin1().constData(),
...
bool TestBase::executePreCreateScripts(const QString &path)
{
    ...
    for (sqlfileslist_it = preCreateFiles.constBegin();
         sqlfileslist_it != preCreateFiles.constEnd(); ++sqlfileslist_it) {
...
void TestBase::createEmptyDb()
{
    ...
    try {
        ...
        message(tr("Error while creating the table rules, triggers and setting the permissions: %1")
                    .arg(e.error().text()));
        return;
    }
}

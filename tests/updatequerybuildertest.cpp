...
private Q_SLOTS:
    void initTestCase()
    {
        ...
        QSqlQuery query;
        QVERIFY(
            query.exec(QLatin1String("CREATE TABLE table1 (first VARCHAR(128), second INTEGER)")));
    }
    ...
    void testQueryBuilder_data()
    {
        ...
        values << QL1S("foo") << 42;
        QTest::newRow("2 columns, string and number")
            << qb << "UPDATE table1 SET first = :0, second = :1" << columns << values;
        ...
        columns << QL1S("first") << QL1S("second");
        values << QL1S("foo") << 42;
        QTest::newRow("2 columns, no subtable update")
            << qb << "UPDATE ONLY table1 SET first = :0, second = :1" << columns << values;
        ...
        qb.addColumnValue(QL1S("first"), QL1S("foo"));
        qb.addColumnValue(QL1S("second"), 42);
        qb.whereCondition().addPlaceholderCondition(QL1S("first"), SqlCondition::Equals,
                                                    QL1S(":p"));
        columns.clear();
        values.clear();
        columns << QL1S("first") << QL1S("second");
        values << QL1S("foo") << 42;
        QTest::newRow("Where clause, placeholder")
            << qb << "UPDATE ONLY table1 SET first = :0, second = :1 WHERE first = :p" << columns
            << values;
        ...
        qb.whereCondition().addValueCondition(Workplace.short_desc, SqlCondition::Equals,
                                              QL1S("bar"));
        columns.clear();
        values.clear();
        columns << QL1S("description") << QL1S("itemorder");
        values << QL1S("foo") << 42 << QL1S("bar");
        QTest::newRow("Where clause, value") << qb
                                             << "UPDATE ONLY tblWorkplace SET description = :0, "
                                                "itemorder = :1 WHERE tblWorkplace.short_desc = :2"
                                             << columns << values;
        ...
        values << QVariant();
        QTest::newRow("server-side now")
            << qb << "UPDATE tblReport SET ts = now(), txt = :0" << columns << values;
    }
    ...
    void testQueryBuilder()
    {
        ...
        QVERIFY(std::equal(qb.m_bindValues.begin(), qb.m_bindValues.end(), values.begin(),
                           deepVariantCompare));
    }
};

...
private Q_SLOTS:
    ...
    void testSelect_data()
    {
        ...
        QTest::newRow("single col, no cond")
            << select(Person.id).from(Person).queryBuilder() << "SELECT tblPerson.id FROM tblPerson"
            << QVector<QVariant>();
        ...
        QTest::newRow("single condition, less than")
            << select(Person.id)
                   .from(Person)
                   .where(Person.Hired < QDateTime(QDate(2013, 10, 28)))
                   .queryBuilder()
...
#pragma BullseyeCoverage off
        ...
        QTest::newRow("double condition")
            << select(Person.id)
                   .from(Person)
                   .where(Person.HireRights == true
                          || Person.PersonForename == Person.PersonSurname)
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson WHERE (tblPerson.HireRights = :0 OR "
               "tblPerson.PersonForename = tblPerson.PersonSurname)"
            << (QVector<QVariant>() << true);

        QTest::newRow("multi col, multi cond")
            << select(Person.id, Person.PersonForename, Person.PersonSurname)
                   .from(Person)
                   .where(Person.PersonSurname == Person.PersonForename
                          && Person.PersonActive == true)
                   .queryBuilder()
            << "SELECT tblPerson.id, tblPerson.PersonForename, tblPerson.PersonSurname "
               "FROM tblPerson WHERE (tblPerson.PersonSurname = tblPerson.PersonForename AND "
               "tblPerson.PersonActive = :0)"
            << (QVector<QVariant>() << 1);
#ifdef _BullseyeCoverage
#pragma BullseyeCoverage on
        ...
        QTest::newRow("single join")
            << select(Person.id, PersonGrades.description)
                   .from(Person)
                   .innerJoin(PersonGrades, Person.PersonGrade == PersonGrades.id)
                   .queryBuilder()
            << "SELECT tblPerson.id, lutPersonGrades.description FROM tblPerson INNER JOIN "
               "lutPersonGrades ON tblPerson.fk_lutPersonGrades_id = lutPersonGrades.id"
            << QVector<QVariant>();

        QTest::newRow("double join")
            << select(Person.id)
                   .from(Person)
                   .innerJoin(PersonSubDirectoratesRelation,
                              Person.id == PersonSubDirectoratesRelation.left)
                   .innerJoin(SubDirectorates,
                              PersonSubDirectoratesRelation.right == SubDirectorates.id)
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson INNER JOIN rltPersonSubDirectorates ON "
               "tblPerson.id = rltPersonSubDirectorates.fk_tblPerson_id "
               "INNER JOIN lutSubDirectorates ON rltPersonSubDirectorates.fk_lutSubDirectorates_id "
               "= lutSubDirectorates.id"
            << QVector<QVariant>();
        ...
        QTest::newRow("double join with condtion")
            << select(Person.id)
                   ...
                   .innerJoin(SubDirectorates,
                              PersonSubDirectoratesRelation.right == SubDirectorates.id)
                   .where(SubDirectorates.description == QString::fromLatin1("foo"))
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson INNER JOIN rltPersonSubDirectorates ON "
               "tblPerson.id = rltPersonSubDirectorates.fk_tblPerson_id "
               "INNER JOIN lutSubDirectorates ON rltPersonSubDirectorates.fk_lutSubDirectorates_id "
        ...
        QTest::newRow("leftOuterJoin")
            << select(Person.id, PersonGrades.description)
                   .from(Person)
                   .leftOuterJoin(PersonGrades, Person.PersonGrade == PersonGrades.id)
                   .queryBuilder()
            << "SELECT tblPerson.id, lutPersonGrades.description FROM tblPerson LEFT OUTER JOIN "
               "lutPersonGrades ON tblPerson.fk_lutPersonGrades_id = lutPersonGrades.id"
            << QVector<QVariant>();
        ...
        QTest::newRow("single order by, desc")
            << select(Person.id)
                   .from(Person)
                   .orderBy(Person.PersonSurname, Qt::DescendingOrder)
                   .queryBuilder()
        ...
        QTest::newRow("double order by, default")
            << select(Person.id)
                   .from(Person)
                   .orderBy(Person.PersonSurname, Person.PersonForename)
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson ORDER BY tblPerson.PersonSurname ASC, "
               "tblPerson.PersonForename ASC"
            << QVector<QVariant>();

        QTest::newRow("double order by, mixed")
            << select(Person.id)
                   .from(Person)
                   .orderBy(Person.PersonSurname, Qt::AscendingOrder, Person.PersonForename,
                            Qt::DescendingOrder)
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson ORDER BY tblPerson.PersonSurname ASC, "
               "tblPerson.PersonForename DESC"
            << QVector<QVariant>();
...
#pragma BullseyeCoverage off
        ...
        QTest::newRow("placeholder + value condition")
            << select(Person.id)
                   .from(Person)
                   .where(Person.id == placeholder(":foo")
                          || Person.PersonSurname == QString::fromLatin1("bar"))
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson WHERE (tblPerson.id = :foo OR "
               "tblPerson.PersonSurname = :0)"
...
#pragma BullseyeCoverage on
        ...
        QTest::newRow("LIKE condition")
            << select(Person.id)
                   .from(Person)
                   .where(like(Person.PersonForename, QLatin1String("foo%")))
                   .queryBuilder()
        ...
        QTest::newRow("1 GROUP BY")
            << select(Person.id)
                   .from(Person)
                   .groupBy(Person.PersonSurname, Person.id)
                   .queryBuilder()
        ...
        QTest::newRow("2 GROUP BY")
            << select(Person.id)
                   .from(Person)
                   .groupBy(Person.PersonSurname, Person.PersonForename, Person.id)
                   .queryBuilder()
            << "SELECT tblPerson.id FROM tblPerson GROUP BY tblPerson.PersonSurname, "
               "tblPerson.PersonForename, tblPerson.id"
            << QVector<QVariant>();
    }

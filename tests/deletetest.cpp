...
private Q_SLOTS:
    ...
    void testDelete_data()
    {
        ...
        QTest::newRow("delete rows, one condition")
            << del()
                   .from(Person)
                   .where(Person.PersonSurname == QLatin1String("Ford"))
                   .queryBuilder()
        ...
        QTest::newRow("delete rows, multiple conditions")
            << del()
                   .from(Person)
                   .where(Person.PersonSurname == QLatin1String("Ford")
                          && Person.PersonForename == QLatin1String("Gerald"))
                   .queryBuilder()
            << "DELETE FROM tblPerson WHERE (tblPerson.PersonSurname = :0 AND "
               "tblPerson.PersonForename = :1)"
            << (QVector<QVariant>() << QLatin1String("Ford") << QLatin1String("Gerald"));

        QTest::newRow("delete all rows")
            << del().from(Person).queryBuilder() << "DELETE FROM tblPerson" << QVector<QVariant>();
    }

...
namespace detail {
    ...
    {
        notification_rule_creator(QStringList &stmts)
            : m_stmts(stmts)
        {
        }
        template <typename T>
        void operator()(wrap<T>)
        {
            ...
            const QStringList ops = QStringList()
                << QLatin1String("Insert") << QLatin1String("Update") << QLatin1String("Delete");
            foreach (const QString &op, ops) {
                const QString stmt = QLatin1Literal("CREATE OR REPLACE RULE ") % T::tableName()
                    % QLatin1Literal("Notification") % op % QLatin1Literal("Rule AS ON ")
                    % op.toUpper() % QLatin1Literal(" TO ") % T::tableName()
                    % QLatin1Literal(" DO ALSO NOTIFY ") % T::tableName()
    ...
    {
        column_notification_rule_creator(QStringList &stmts)
            : m_stmts(stmts)
        {
        }
        template <typename T>
        void operator()(wrap<T>)
        {
            if (T::notify::value) // notify with the corresponding column content ( as the notify
                                  // channel) when a row is modified
            {
                // be sure to have unique rule name and stay within the allowed name size. We don't
                // care that it's cryptic as it isn't used anywhere. so use Uuids.
                QString ruleID = QUuid::createUuid().toString();
                ruleID.replace(QLatin1Char('{'), QLatin1Char('\"'));
                ruleID.replace(QLatin1Char('}'), QLatin1Char('\"'));

                const QString identifier = SqlUtils::createIdentifier(
                    T::table::sqlName() % QLatin1Literal("_") % T::sqlName());
                const QString stmt = QLatin1Literal("CREATE OR REPLACE RULE ") % ruleID
                    % QLatin1Literal(" AS ON UPDATE TO ") % T::table::sqlName()
    ...
    /**
     * Table-wise notification rule creator (create statements for each Notify-enabled column in all
     * tables).
     * @internal
     */
    struct table_notification_rule_creator
    {
        table_notification_rule_creator(QStringList &stmts)
            : m_stmts(stmts)
        {
        }
        template <typename T>
        void operator()(wrap<T>)
        {
            boost::mpl::for_each<typename T::columns, detail::wrap<boost::mpl::placeholders::_1>>(
                column_notification_rule_creator(m_stmts));
        }
        QStringList &m_stmts;
    };
} // namespace detail
...
QStringList createNotificationRuleStatements()
{
    ...
    boost::mpl::for_each<Schema, detail::wrap<boost::mpl::placeholders::_1>>(
        detail::notification_rule_creator(statements));
    boost::mpl::for_each<Schema, detail::wrap<boost::mpl::placeholders::_1>>(
        detail::table_notification_rule_creator(statements));

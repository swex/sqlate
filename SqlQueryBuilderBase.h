...
public:
    ...
    /// prepared Subclasses must implement this method. The method throws an SqlException if there
    /// is an error preparing the query.
    virtual SqlQuery &query() = 0;

    /// Resets the internal status to "not assembled", meaning the query() call will assemble the
    /// query again. This makes possible to modify an already existing builder object after query()
    /// was used.
    void invalidateQuery();

protected:
    /** Binds @p value to placeholder @p placeholderIndex in m_query.
     *  Unlike the similar methods in QSqlQuery this also applies transformations to handle types
     * not supported by QtSQL, such as UUID.
     *  @note This assumes the use of named bindings in the form ":&lt;index&gt;".
     */
    void bindValue(int placeholderIndex, const QVariant &value);

    /** Returns the SQL expression returning the current date/time on the server depending on the
     * used database backend. */
